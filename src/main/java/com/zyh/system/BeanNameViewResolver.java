...
     *（通常在创建实际View对象时出现问题）
    ...
    /* XXX
      ...
      BeanFactory和FactoryBean其实没有什么比较性的，只是两者的名称特别接近，
                  所以有时候会拿出来比较一番，BeanFactory是提供了OC容器最基本的形式，给具体的IOC容器的实现提供了规范，
      ...
      FactoryBean在IOC容器的基础上给Bean的实现加上了一个简单工厂模式和装饰模式，
                  ...
                  要想深入准确的理解FactoryBean,只有去读读Spring源码了。
     ...
     *
                     在讲SpringMVC容器创建时介绍过WebApplicationContext是在FrameworkServlet中创建的，默认使用的是XmlWebApplicationContext，
                     它的父类是AbstractRefreshableWebApplicationContext，这个类实现了ThemeSource接口，实现方式是在内部封装了一个ThemeSource属性，然后将具体工作交给它。
