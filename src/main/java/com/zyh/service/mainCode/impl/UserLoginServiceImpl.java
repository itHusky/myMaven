...
public class UserLoginServiceImpl extends BaseServiceImpl<UserLogin, Integer> implements
     ...
     * 用户登录验证
     *
     * MD5不可逆的原因是其是一种散列函数，使用的是hash算法，
     * 在计算过程中原文的部分信息是丢失了的，不过有个地方值得指出的是，
     * 一个MD5理论上的确是可能对应无数多个原文的，
     * 因为MD5是有限多个的而原文可以是无数多个。
     ...
     * 因此某种意义上来说，在一定范围内想构建MD5值与原文的一一对应关系是完全有可能的。
     * 所以对于MD5目前最有效的攻击方式就是彩虹表_百度百科。
     *
     */
    @Override
